...
import type { HeroSlide } from '@/components/shared/HeroCarousel';
...
export default function HomePage() {
  ...
  const handleSubmitProject = () => {
    if (isAuthenticated) {
      ...
      router.push('/login?redirect=/pr/create');
    }
  };
  ...
  return (
    <Box sx={{ flexGrow: 1, minHeight: '100vh', bgcolor: theme.palette.background.default }}>
            ...
            <Grid container spacing={3} sx={{ mb: 6 }}>
              {recentProjects.map((project) => (
                <Grid size={{ xs: 12, md: 6, lg: 4 }} key={project.id}>
                  ...
                  >
                    <CardActionArea onClick={() => router.push(`/projects/${project.slug || project.id}`)}>
                      <Box
                        sx={{
                          height: 120,
      ...
      <SectionBand
        ...
        ctaAction={isAuthenticated ? handleSubmitProject : () => router.push('/register')}
        variant="primary"
        align="center"
      />
    </Box>
