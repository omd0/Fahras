...
export default function EmailVerificationPage() {
  ...
  }, [isAuthenticated, user?.email_verified_at, router]);
  ...
  const handleVerify = async () => {
    ...
    try {
      const response = (await authApi.verifyEmail(email, otp)) as {
        message: string;
        user: User;
        token?: string;
      };
    ...
    >
      <Container maxWidth="sm">
        ...
        >
          ...
          <Box sx={{ mb: 3 }}>
            <OTPInput value={otp} onChange={setOtp} length={6} disabled={loading} error={!!error} />
          </Box>
