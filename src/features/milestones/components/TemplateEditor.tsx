...
import type { MilestoneTemplate, MilestoneTemplateItem, MilestoneTemplateData } from '@/types';
...
export const TemplateEditor: React.FC<TemplateEditorProps> = ({
  ...
  const deriveRolesAndActions = (permissions: Set<string>) => {
    ...
    return {
      roles: Array.from(rolesInPermissions) as ('admin' | 'faculty' | 'student')[],
      actions: Array.from(actionsInPermissions) as (
        | 'start'
        | 'pause'
        | 'extend'
        | 'view'
  ...
  const handleAddItem = () => {
    ...
    const { roles, actions } = deriveRolesAndActions(itemPermissions);
    ...
    const newItem: Partial<MilestoneTemplateItem> & { tempId: number } = {
      ...
      allowed_actions: actions.length > 0 ? actions : undefined,
    };
  ...
  const ROLES = ['admin', 'faculty', 'student'] as const;
  const ACTIONS = ['start', 'pause', 'extend', 'view', 'edit', 'complete'] as const;
        ...
        <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
          ...
          <Box
            sx={{
              ...
              overflowX: 'auto',
              overflowY: 'visible',
              backgroundImage: `radial-gradient(circle, ${theme.palette.grey[400]} 1px, transparent 1px)`,
            ...
            >
              {items.length === 0 ? (
                ...
                >
                  ...
                  <Button variant="contained" startIcon={<AddIcon />} onClick={handleOpenAddStep}>
                    Add Step
                  </Button>
                </Box>
              ) : (
                    ...
                    <Card
                      ...
                      sx={{
                        ...
                        '&:hover': {
                          ...
                          transform:
                            draggedIndex === index ? 'rotate(5deg)' : 'translateY(-2px)',
                          '& .action-buttons': {
                      ...
                      <CardContent sx={{ p: 2, pb: '16px !important' }}>
                        <Box
                          sx={{
                            ...
                            '&:hover': { opacity: 1, color: theme.palette.text.secondary },
                          }}
                        >
              ...
              <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2, pt: 1 }}>
                ...
                <Box>
                  ...
                  <FormHelperText sx={{ mt: 1 }}>
                    Check the boxes to grant permissions. Each checkbox represents a role-action
                    combination.
                  </FormHelperText>
                </Box>
              </Box>
