...
export const FacultyDashboard: React.FC = () => {
  ...
  const isMyAdvisingProject = useCallback(
    (project: Project) => {
      const advisors = project.advisors || [];
      const isAdvisor = advisors.some((advisor) => advisor?.id === user?.id);
      const createdByCurrentFaculty =
        userIsFaculty &&
        (project.created_by_user_id === user?.id || project.creator?.id === user?.id);

      return isAdvisor || createdByCurrentFaculty;
    },
    [user?.id, userIsFaculty],
  );
            ...
            <CardContent>
              ...
              >
                ...
                <Typography variant="body2" color="text.secondary">
                  ...
                  {stats.advisingProjects === 1 ? t('project') : t('projects')}
                </Typography>
              </Box>
              ...
              {(projects || []).length === 0 && (
                <Box sx={{ textAlign: 'center', py: 6 }}>
                  <SchoolIcon
                    sx={{ fontSize: 64, color: 'text.secondary', opacity: 0.3, mb: 2 }}
                  />
                  <Alert severity="info">{t('No projects available yet.')}</Alert>
                </Box>
              )}
            </CardContent>
          </Card>
