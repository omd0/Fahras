...
export const TVTCLogo: React.FC<TVTCLogoProps> = ({
  ...
  if (variant === 'icon') {
    return <SchoolIcon sx={{ fontSize: iconSize, ...colorStyles, ...sx }} />;
  }
  ...
  if (variant === 'text') {
    return (
      <Typography
        ...
        sx={{ fontWeight: 700, ...colorStyles, ...sx }}
      >
        TVTC
      </Typography>
    );
  ...
  return (
    <Box sx={{ display: 'flex', alignItems: 'center', ...sx }}>
      <SchoolIcon sx={{ fontSize: iconSize, mr: 1, ...colorStyles }} />
      <Typography
        variant={size === 'large' ? 'h4' : size === 'small' ? 'h6' : 'h5'}
        sx={{ fontWeight: 700, ...colorStyles }}
      >
        TVTC
      </Typography>
    </Box>
