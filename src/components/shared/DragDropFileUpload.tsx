  ...
  const validateFile = useCallback(
    (file: File): { valid: boolean; error?: string } => {
      if (file.size > maxSizeBytes) {
        return { valid: false, error: `File "${file.name}" exceeds maximum size of ${formatFileSize(maxSizeBytes)}` };
      }
      ...
      if (acceptedTypes.length > 0) {
        ...
        if (!isAccepted) {
          return { valid: false, error: `File "${file.name}" has unsupported type. Accepted: ${acceptedTypes.join(', ')}` };
        }
      }
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4, ease: 'easeOut' }}>
        ...
        >
          ...
          <Box sx={{ position: 'relative', zIndex: 1 }}>
            <motion.div variants={iconVariants} animate={isDragging ? 'dragging' : 'idle'}>
              <Avatar
                sx={{
                  width: 72,
                  height: 72,
            ...
            <Box sx={{ display: 'flex', gap: 1, justifyContent: 'center', flexWrap: 'wrap' }}>
              <Chip size="small" label={`Max ${maxFiles} files`} variant="outlined" sx={{ fontWeight: 500 }} />
              <Chip size="small" label={`Max ${formatFileSize(maxSizeBytes)}`} variant="outlined" sx={{ fontWeight: 500 }} />
              {acceptedTypes.length > 0 && (
                <Chip size="small" label={`Accepted: ${acceptedTypes.join(', ')}`} variant="outlined" sx={{ fontWeight: 500 }} />
              )}
            </Box>
          </Box>
        </Paper>
            ...
            <List sx={{ p: 0 }}>
              <AnimatePresence mode="popLayout">
                {files.map((file, index) => {
                  ...
                  return (
                    <motion.div key={`${file.name}-${index}`} variants={fileItemVariants} initial="hidden" animate="visible" exit="exit" layout>
                      <Paper
                        elevation={0}
                        sx={{
                          mb: 1.5,
                          ...
                          '&:hover': { boxShadow: '0 4px 12px rgba(0,0,0,0.1)', transform: 'translateX(4px)' },
                        }}
                      >
                        <ListItem sx={{ py: 1.5, px: 2 }}>
                          <ListItemAvatar>
                            <Avatar sx={{ bgcolor: isError ? 'error.light' : isSuccess ? 'success.light' : 'primary.light' }}>
                              {isSuccess ? (
                            ...
                            secondary={
                              <Box>
                                <Box sx={{ display: 'flex', gap: 1, mt: 0.5, mb: showProgress && isUploading ? 1 : 0 }}>
                                  <Chip size="small" label={formatFileSize(file.size)} sx={{ height: 20, fontSize: '0.75rem' }} />
                                  <Chip size="small" label={file.type || 'Unknown type'} sx={{ height: 20, fontSize: '0.75rem' }} />
                                  {isUploading && fileProgress && (
                                    <Chip size="small" label={`${fileProgress.progress}%`} color="primary" sx={{ height: 20, fontSize: '0.75rem' }} />
                                  )}
                                  {isSuccess && <Chip size="small" label="Uploaded" color="success" sx={{ height: 20, fontSize: '0.75rem' }} />}
                                  {isError && <Chip size="small" label="Failed" color="error" sx={{ height: 20, fontSize: '0.75rem' }} />}
                                </Box>
                                {showProgress && isUploading && fileProgress && (
                                  <LinearProgress variant="determinate" value={fileProgress.progress} sx={{ mt: 1, borderRadius: 1, height: 6 }} />
                                )}
