...
class ApiService {
  ...
  async getSavedSearches(): Promise<{ success: boolean; data: SavedSearch[] }> {
    const response: AxiosResponse = await this.api.get('/saved-searches');
    return response.data;
  }

  async getSavedSearch(searchId: number): Promise<{ success: boolean; data: SavedSearch }> {
    const response: AxiosResponse = await this.api.get(`/saved-searches/${searchId}`);
    return response.data;
  }

  async createSavedSearch(data: CreateSavedSearchData): Promise<{ success: boolean; message: string; data: SavedSearch }> {
    const response: AxiosResponse = await this.api.post('/saved-searches', data);
    return response.data;
  }

  async updateSavedSearch(searchId: number, data: UpdateSavedSearchData): Promise<{ success: boolean; message: string; data: SavedSearch }> {
    const response: AxiosResponse = await this.api.put(`/saved-searches/${searchId}`, data);
    return response.data;
  }

  async deleteSavedSearch(searchId: number): Promise<{ success: boolean; message: string }> {
    const response: AxiosResponse = await this.api.delete(`/saved-searches/${searchId}`);
    return response.data;
  }

  async recordSavedSearchUsage(searchId: number): Promise<{ success: boolean; message: string; data: SavedSearch }> {
    const response: AxiosResponse = await this.api.post(`/saved-searches/${searchId}/use`);
    return response.data;
  }

  async setSavedSearchAsDefault(searchId: number): Promise<{ success: boolean; message: string; data: SavedSearch }> {
    const response: AxiosResponse = await this.api.post(`/saved-searches/${searchId}/set-default`);
    return response.data;
  }
}
